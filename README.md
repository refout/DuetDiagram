...
| Phase 0a | 依赖版本核实、界面栈自检出图、两款布局引擎并排取证、布局约束补齐、协议传输与自定义字段验证 |

**未实现**：布局引擎封装、Sidecar、Mermaid / DSL、渲染与画布、外部代理接入。
详见 `docs/Architecture.md`。

外部代理接入的协议链路已经验证过（`reports/phase0a-mcp.md`）：本地子进程、
网络长连接、网络短请求三条传输都可用；但会话建立请求在当前协议版本下换了名字，
自定义字段要走能力声明的扩展位，这两点与最初的设想不同。
