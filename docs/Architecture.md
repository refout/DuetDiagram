...
| `DuetDiagram.Render` | 四叉树空间索引与视口剔除、绘制列表（IR 与布局结果翻译成有序绘制指令）、视口变换与视口状态。画布控件在主程序 |
...
| `DuetDiagram.E2E.Tests` | 无头模式下的端到端用例：起窗口、送输入、抓一帧 |

## 未实现模块

| 模块 | 方案位置 | 计划 |
|---|---|---|
| 视口虚拟化（剔除与动态阈值切换）、诊断面板、属性面板与其余分区 | §九 GUI 设计、§十 性能策略 | Phase 2 |
