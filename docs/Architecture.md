...
| `DuetDiagram.Llm` | 八个粗粒度工具的定义、参数 schema 与参数校验；一份定义两处派生（模型侧要函数，代理侧要由同一个函数派生出来的工具）；上下文摘要与 `diagram_read`。其余动作分发表还没接上的会返回结构化的「尚未接上」 |
...
| 八个工具的动作分发表（改结构、改外观、布局、组合、导出、校验、撤销重做各自的执行体） | §6.1 | Phase 3 |
