...
`LAYER_FORBIDDEN` 与 `MCP_FORBIDDEN` 分开：那一条是"整份文档都不许改"，处置是换一份凭据；
这一条是"能改，但不能改这一层"，处置是换一个图层，或者去改那份凭据的图层范围。
合成一个的话，调用方会去换凭据，而换来的那一份照样够不着这一层。
它由工具层产生，因为一次写入点名了哪个图层只有解析过动作参数的那一层知道：
改节点归属时图层写在 `value` 上，建图层与改图层时写在 `id` 上。
