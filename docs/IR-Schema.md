...
`shape` 字段存的是**身份**（`NodeShape` 枚举，见上面 `NodeDef` 那张表），不是几何。
几何在 `Shapes/` 里那份形状表上：`ShapeRegistry.Default.Find(shape).Geometry`。

- **枚举不许删。** 八个成员已经写进序列化与结构哈希，删掉一个就是一次数据迁移。
  形状库的做法是"枚举保留、几何搬到表里"，两边由注册表在构造时校验一致——
  表少一个枚举值直接抛，不让它拖到渲染时才出问题。
- **几何用单位框坐标**（0 到 1），不含任何绘制库类型。Core 不依赖 Avalonia，
  所以画布、形状面板的预览、将来的导出器画的是同一份几何。
- **四类轮廓**：`RoundedRectOutline`（直角 / 圆角 / 胶囊，差别只在圆角从哪来）、
...
模板是**一份可复用的文档片段**：一组节点、边与组合，连同它们的样式。
**它是文档片段，不是第二套 IR**——文件就是一份少了几个集合的文档，读写都走
`DiagramSerializer`。另造一套格式的话，IR 每加一个字段就有两个地方要改，
而漏改的那一处只在读到新文件时才暴露。

- **只带节点、边与组合。** 页面、图层、标签、动作、字体、文本预设、调色板与布局提示
  都是文档级的东西：拼进另一份文档时要么没有落点（标签、动作），要么需要一套合并规则
  （图层、调色板），要么本来就该跟着目标文档走（页面、画布）。带了这些的文件被**拒绝**
  而不是静默丢掉——静默丢掉的表现是"模板里的颜色不见了"，
  而用户无从知道是模板没写对还是程序没做。
- **名字取自文件里的文档标识**，模板不另立一套命名：多一个字段就多一处可能与文件名对不上的地方。
- **标识冲突要消解。** 片段里的标识在目标文档里可能已经存在。冲突时在后头接一个序号
  （**从 2 起**，原名占的就是第一个），**引用一起跟着改**——只改节点名不改边上的引用的话，
  拼进去的是一条指向旧名字的边。试序号时把**这次计划里已经分配出去的**也算上，
  否则片段里同时有 `a` 与 `a-2` 而文档里已有 `a` 时，两个都会变成 `a-2`。
- **组合的成员表是权威，元素的父级只是它的冗余索引**（与整体校验同一个口径，见「一致性校验」）。
  于是拼进去时父级由成员表反推，而不是照抄片段里写的那一份；片段里两份对不上时拒绝——
  一个元素只能有一个父级，那两份里必然有一份落不了地。
- **片段自己的引用要核。** 边指向片段里没有的端点、成员表里的标识片段里没有、
  父级指向片段里没有的组合，都在拼之前报出来。
- **嵌套深度合起来看。** 片段自己没超，拼到一份已经很深的文档上照样会超；
  这一步不做的话，插入成功而文档立刻变成一份校验不过的东西。
- **页面归属在截取那一步就去掉**，实例化时再清一次。带着来源文档的页号的话，
  放进第二页的东西会被模板带回第一页。
- **缺字段按声明默认补齐。** 手写的片段 JSON 会省掉不写的字段，而源生成的序列化器
  走的是全参构造器——省掉的键会读成 `default`（引用类型是 `null`）而不是声明处的初值。
  `DiagramDocument` 的构造入口逐字段补齐，让稀疏文件读出来的元素与正常写出来的一样。

模板的解析与装配在 Core（`Templates/`），目录发现与面板在宿主（`DuetDiagram.App/Templates/`，
